...
namespace UIWidgets
{
	...
	public class ListView : ListViewBase {
		...
		{
			return Enumerable.Range(0, strings.Count)
				.Where(i => strings[i]==item)
				.ToList();
		}
		...
		public virtual int Add(string item)
		{
			...
			{
				var old_indicies = FindIndicies(item);
				...
				var diff = new_indicies.Except(old_indicies).ToList();
				var index = diff.Count > 0 ? diff[0] : new_indicies.FirstOrDefault();
		...
		List<string> FilterItems(List<string> newItems)
		{
			IEnumerable<string> temp = newItems;
